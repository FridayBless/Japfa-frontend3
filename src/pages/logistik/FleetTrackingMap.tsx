...
export default function FleetTrackingMap() {
    ...
    const selectTruck = (id: string) => {
        ...
        else { setSelectedTruck(id); const t = dummyFleet.find(x => x.id === id); if (t) flyTo(t.lon, t.lat); }
    };

    const routesGJ = useMemo(() => buildRoutesGeoJSON(dummyFleet, selectedTruck), [selectedTruck]);
    const geoGJ = useMemo(() => buildGeofencesGeoJSON(dummyFleet, selectedTruck), [selectedTruck]);
    const zonesGJ = useMemo(() => buildZonesGeoJSON(), []);

    const stats = useMemo(() => ({
        active: dummyFleet.filter(t => t.status === 'in-transit').length,
        delayed: dummyFleet.filter(t => t.status === 'delayed').length,
        idle: dummyFleet.filter(t => t.status === 'idle').length,
        totalStops: dummyFleet.reduce((s, t) => s + t.customers.length, 0),
        done: dummyFleet.reduce((s, t) => s + t.customers.filter(c => c.delivered).length, 0),
    }), []);

    return (
        <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col" style={{ height: '780px' }}>
            <style>{css}</style>
            ...
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-3 bg-slate-50/50 dark:bg-[#222]/50">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">7 UNITS</span>
                </div>
            ...
            <div className="flex-1 flex relative">
                ...
                <div className="flex-1 relative">
                    <Map ref={mapRef} {...viewState} onMove={(e: any) => setViewState(e.viewState)} style={{ width: '100%', height: '100%' }} mapStyle="mapbox://styles/mapbox/navigation-night-v1" mapboxAccessToken={import.meta.env.VITE_MAPBOX_TOKEN}>
                        {/* Zone Polygons */}
                        {showZones && (
                        ...
                        {dummyFleet.map((truck) => {
                            const isDim = selectedTruck !== null && selectedTruck !== truck.id;
                            return truck.customers.map((c, j) => (
                                <Marker key={`${truck.id}-c${j}`} longitude={c.lon} latitude={c.lat} anchor="center" onClick={(e) => { e.originalEvent.stopPropagation(); setPopupInfo({ type: 'customer', customer: c, truck }); }}>
                        ...
                        {dummyFleet.map((truck) => {
                            ...
                            return (
                                <Marker key={truck.id} longitude={truck.lon} latitude={truck.lat} anchor="center" onClick={(e) => { e.originalEvent.stopPropagation(); setPopupInfo({ type: 'truck', truck }); selectTruck(truck.id); }}>
                                    <div className={`tk-pin ${isActive ? 'tk-active' : ''} ${isDim ? 'tk-dim' : ''}`}>
                                        ...
                                        {!isDim && (
                                            <div style={{ position: 'absolute', top: -22, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap', background: '#0f172a', color: 'white', fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 4, boxShadow: '0 2px 6px rgba(0,0,0,0.4)' }}>
                                                {truck.plate} <span style={{ color: statusColor(truck.status) }}>● {delivered}/{truck.customers.length}</span>
                                            </div>
                                        )}
                                    </div>
                                </Marker>
                        ...
                        {/* Zone Labels */}
                        {showZones && !selectedTruck && dummyFleet.length > 0 && (
                            <>
                                {[
                            ...
                            <Popup longitude={DEPO_LON} latitude={DEPO_LAT} onClose={() => setPopupInfo(null)} closeOnClick={false} anchor="bottom" className="custom-popup">
                                <div className="p-2 min-w-[200px]">
                                    ...
                                    <div className="text-xs text-slate-500 space-y-1">
                                        ...
                                        <div className="flex justify-between"><span>Trucks Dispatched</span><b className="text-slate-800">{stats.active + stats.delayed}/7</b></div>
                        ...
                        {popupInfo && popupInfo.type === 'truck' && (
                            <Popup longitude={popupInfo.truck.lon} latitude={popupInfo.truck.lat} onClose={() => setPopupInfo(null)} closeOnClick={false} anchor="bottom" className="custom-popup">
                        ...
                        {popupInfo && popupInfo.type === 'customer' && (
                            <Popup longitude={popupInfo.customer.lon} latitude={popupInfo.customer.lat} onClose={() => setPopupInfo(null)} closeOnClick={false} anchor="bottom" className="custom-popup">
                                <div className="p-2 min-w-[220px] space-y-2">
                                    ...
                                    <div className="text-xs text-slate-500 space-y-1">
                                        ...
                                        <div className="flex justify-between"><span>Muatan</span><b className="text-slate-800">{popupInfo.customer.weightKg} KG</b></div>
                ...
                <div className="w-[300px] border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1a1a1a] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2 fp">
                        {dummyFleet.map((truck) => {
                            ...
                            const delivered = truck.customers.filter(c => c.delivered).length;
                            const pct = Math.round((delivered / truck.customers.length) * 100);
                            return (
                                <div key={truck.id} onClick={() => selectTruck(truck.id)}
                                    ...
                                    <div className="mt-2.5 flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-500 shrink-0">{delivered}/{truck.customers.length}</span>
                                    </div>
                                    {/* Mini stats */}
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-[#222]/50 space-y-2">
                        ...
                        <div className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-primary to-emerald-500 rounded-full transition-all" style={{ width: `${(stats.done / stats.totalStops) * 100}%` }}></div>
                        </div>
                        <div className="flex justify-between text-[10px]"><span className="text-slate-400">{Math.round((stats.done / stats.totalStops) * 100)}% Complete</span><span className="text-slate-400">{stats.totalStops - stats.done} remaining</span></div>
                    </div>
                </div>
            </div>
        </div>
