...
export default function KasirHistory() {
    ...
    useEffect(() => {
        ...
        const parsed: ExpenseEntry[] = saved ? JSON.parse(saved) : [];
        
        const hasDummy = parsed.some(e => e.id.startsWith('dummy-'));

        if (!hasDummy) {
            // Generate 10 dummy data entries
            const dummyData: ExpenseEntry[] = Array.from({ length: 10 }).map((_, i) => {
                const d = new Date();
                d.setDate(d.getDate() - (i % 7)); // spread over last 7 days
                
                const bbm = 150000 + (i * 25000);
                const tol = 50000 + (i * 10000);
                const parkir = 10000;
                const parkirLiar = 5000;
                const kuliAngkut = 20000;
                const lainLain = 50000;
                
                return {
                    id: `dummy-${i}-${Date.now()}`,
                    time: `0${8 + (i % 4)}:00`,
                    date: d.toISOString().split('T')[0],
                    plate: `B ${1234 + i * 111} JPF`,
                    vehicleType: i % 2 === 0 ? 'Fuso' : 'CDE',
                    driver: `Sopir Dummy ${i + 1}`,
                    isOncall: i % 3 === 0,
                    bbm,
                    tol,
                    parkir,
                    ...
                    kuliAngkut,
                    lainLain,
                    helperName: i % 2 === 0 ? `Helper Dummy ${i + 1}` : '',
                    notes: 'Data dummy untuk demo',
                    total: bbm + tol + parkir + parkirLiar + kuliAngkut + lainLain
                };
            });
            
            const newData = [...parsed, ...dummyData];
            setEntries(newData);
            localStorage.setItem('kasir_expenses', JSON.stringify(newData));
        } else {
            setEntries(parsed);
        }
    }, []);
    ...
    const filteredEntries = entries.filter(e => {
        ...
        // Include entries that fall exactly on the start or end dates
        return entryDate >= start && entryDate <= end;
    });

    const totalExpense = filteredEntries.reduce((sum, e) => sum + e.total, 0);
